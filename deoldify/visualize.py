...
class VideoColorizer:
    ...
    def _download_video_from_url(self, source_url, source_path: Path):
        ...
        ydl_opts = {
            'format': 'bestvideo[ext=mkv]+bestaudio[ext=m4a]/mkv',
    ...
    def _extract_raw_frames(self, source_path: Path):
        ...
        bwframe_path_template = str(bwframes_folder / 'f%3d.png')
        ...
        process = (
            ffmpeg
                ...
                .output(str(bwframe_path_template), format='image2', vcodec='png', **{'q:v':'0'})
    ...
    def _build_video(self, source_path: Path) -> Path:
        colorized_path = self.result_folder / (
            source_path.name.replace('.mkv', '_no_audio.mkv')
        )
        colorframes_folder = self.colorframes_root / (source_path.stem)
        colorframes_path_template = str(colorframes_folder / 'f%3d.png')
        ...
        process = (
            ffmpeg 
                .input(str(colorframes_path_template), format='image2', vcodec='png', framerate=fps) 
        ...
        audio_file = Path(str(source_path).replace('.mkv', '.aac'))
...
# /root/autodl-tmp/DeOldify-master/data/bwframes/models/VideoModel_gen_1_0_2300
# ColorizeStable_gen
# final_model

def get_stable_video_colorizer(
    root_folder: Path = Path('./'),
    weights_name: str = '/root/autodl-tmp/DeOldify-master/data/bwframes/models/final_model',
    results_dir='result_images',
...
# '/root/autodl-tmp/DeOldify-master/data/bwframes/models/resnet101_v2'
# /root/autodl-tmp/DeOldify-master/models/ColorizeStable_gen
def get_stable_image_colorizer(
    root_folder: Path = Path('./'),
    weights_name: str = '/root/autodl-tmp/DeOldify-master/models/ColorizeStable_gen',
    results_dir='result_images',
    render_factor: int = 35
) -> ModelImageVisualizer:
    learn = gen_inference_wide(root_folder=root_folder, weights_name=weights_name)
    ...
    ipythondisplay.display(
        HTML(
            data='''<video alt="test" autoplay 
                ...
                <source src="data:video/mkv;base64,{0}" type="video/mkv" />
             </video>'''.format(
