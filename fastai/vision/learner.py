...
_swin_meta     = {'cut':-2, 'split':_default_split }
...
def create_body(arch:Callable, pretrained:bool=True, cut:Optional[Union[int, Callable]]=None):
    ...
    # model = arch(weights=models.Swin_S_Weights.IMAGENET1K_V1)
    # model = arch(weights=models.ResNet101_Weights.IMAGENET1K_V2)

    model = arch(pretrained=False)
    # state_dict = torch.load('/root/autodl-tmp/DeOldify-master/models/resnet101-63fe2227.pth')
    state_dict = torch.load('/root/autodl-tmp/DeOldify-master/models/resnet101-cd907fc2.pth')
...
def create_body_transformer(arch:Callable, pretrained:bool=True, cut:Optional[Union[int, Callable]]=None):
    "Cut off the body of a typically pretrained `model` at `cut` (int) or cut the model as specified by `cut(model)` (function)."
    model = arch(weights=models.Swin_S_Weights.IMAGENET1K_V1)
    # model = arch(weights=models.ResNet101_Weights.IMAGENET1K_V2)
